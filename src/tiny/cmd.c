 ...
 * - Uses the kernel to execute other executables from the
 *   /TinyDOS/System64/ directory.
 *
 * To compile:
...
#include <fcntl.h>
#include <unistd.h>
/* #include <elf.h>  - REMOVED */
/* #include <sys/mman.h> - REMOVED */
#include <sys/wait.h>
...
/* STACK DEFINITIONS - REMOVED */

// --- Function Prototypes ---
/* void load_and_run_elf(...); - REMOVED */
...
int main() {
    ...
    while (1) {
        ...
        if (strcmp(command, "?") == 0 || strcmp(command, "help") == 0) {
            ...
            // --- External Command Execution via Kernel (fork/exec) ---
            ...
            pid_t pid = fork();
            if (pid == 0) {
                // In Child Process: ask the kernel to execute the command
